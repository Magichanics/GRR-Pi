...
def main(start, end, save_path='path.png'):
    ...
    maze = deepcopy(tempMaze.grid)

    for y in range(len(tempMaze.grid)):
        for x in range(len(tempMaze.grid[y])):
            if tempMaze.grid[y][x] == "g" or tempMaze.grid[y][x] == "y" or tempMaze.grid[y][x] == "R":
                maze[y][x] = 0
            else:
                maze[y][x] = 1
